...
if(isset($_POST['publish'])) {
    ...
    if(!title_exists($title)) {
    $add = "INSERT INTO `seller_notes` (`SellerID`, `Status`, `Title`, `DisplayPicture`, `NumberofPages`, `Description`, `UniversityName`, `Course`, `CourseCode`, `Professor`, `IsPaid`, `SellingPrice`, `NotesPreview`, `CreatedDate`) VALUES ({$_SESSION['userid']}, 11, '{$title}', '{$display_picture}', '{$pages}', '{$description}', '{$university}', '{$course_name}', '{$course_code}', '{$professor}', b'{$ispaid}', '{$sell_price}', '{$note_preview}', now())";
    $add_query = mysqli_query($connection, $add);
    if(!$add_query) {
        die("Query Failed". mysqli_error($connection));
    }
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                    <button type="submit" class="btn btn-primary" data-dismiss="modal" name="yes">YES</button>
                            </div>
                        </div>
                    </div>
                </div>
