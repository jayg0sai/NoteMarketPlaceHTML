...
<?php
                    ...
                    if(isset($_POST['search'])) {
                        ...
                        $download_count = "SELECT * FROM seller_notes WHERE SellerID = '{$_SESSION['userid']}' AND Status = 14 AND (Title LIKE '%$search%' OR Category LIKE '%$search%' OR SellingPrice LIKE '%$search%') ";
                        ...
                        $download_query = "SELECT * FROM seller_notes WHERE SellerID = '{$_SESSION['userid']}' AND Status = 14 AND (Title LIKE '%$search%' OR Category LIKE '%$search%' OR SellingPrice LIKE '%$search%') ORDER BY CreatedDate DESC LIMIT $page1,10 ";
...
?>
    ...
    <nav id="pagination" class="download-p" aria-label="Page navigation example">
        <ul class="pagination">
            <?php
            if($page>=2) {
                   echo '<li id="prev" class="page-item"><a class="page-link" href="myrejected_search.php?page='.($page - 1).'&source='.$search.'"><img src="images/left-arrow.png" alt="arrow"></a></li>';
            }
            
            for($i = 1; $i <= $page_count; $i++) {
                if($i == $page) {
                    echo '<li class="page-item number active"><a class="page-link" href="myrejected_search.php?page='.$i.'&source='.$search.'">'.$i.'</a></li>';
                } else {
                    echo '<li class="page-item number"><a class="page-link" href="myrejected_search.php?page='.$i.'&source='.$search.'">'.$i.'</a></li>';
                }
                
                
            }
            if($page<$page_count)
                   echo '<li id="next" class="page-item"><a class="page-link" href="myrejected_search.php?page='.($page + 1).'&source='.$search.'"><img src="images/right-arrow.png" alt="arrow"></a></li>';
