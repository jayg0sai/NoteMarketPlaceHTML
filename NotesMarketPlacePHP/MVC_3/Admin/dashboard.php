...
<?php
    ...
    $re_q = "SELECT * FROM users WHERE RoleID = 3 AND CreatedDate >= now() - INTERVAL 7 DAY ";
            ...
            <table class="table table-hover" style="width:100%;">
                ...
                <tbody>
                   <?php
                        ...
                        if(isset($_POST['search'])) {
                        ...
                        } elseif(isset($_GET['source']) || isset($_GET['month'])) {
                            $search = $_GET['source'];
                            $month = $_GET['month'];
                            ...
                            $month = "";
                            $download_count = "SELECT * FROM seller_notes WHERE Status = 13 ";
                            $download_query = "SELECT * FROM seller_notes WHERE Status = 13 ORDER BY PublishedDate LIMIT $page1,5 ";
                        }
...
?>
    ...
    <nav id="pagination2" class="publish-p" aria-label="Page navigation example">
        <ul class="pagination">
            <?php
            if($page>=2) {
                   echo '<li id="prev" class="page-item"><a class="page-link" href="dashboard.php?page='.($page - 1).'&source='.$search.'&month='.$month.'"><img src="images/left-arrow.png" alt="arrow"></a></li>';
            }
            
            for($i = 1; $i <= $page_count; $i++) {
                if($i == $page) {
                    echo '<li class="page-item number active"><a class="page-link" href="dashboard.php?page='.$i.'&source='.$search.'&month='.$month.'">'.$i.'</a></li>';
                } else {
                    echo '<li class="page-item number"><a class="page-link" href="dashboard.php?page='.$i.'&source='.$search.'&month='.$month.'">'.$i.'</a></li>';
                }
                
                
            }
            if($page<$page_count)
                   echo '<li id="next" class="page-item"><a class="page-link" href="dashboard.php?page='.($page + 1).'&source='.$search.'&month='.$month.'"><img src="images/right-arrow.png" alt="arrow"></a></li>';
