                        ...
                        <div class="form-group col-md">
                            ...
                            <div class="col">
                                ...
                                <select class="form-control select" id="note" name="note">
                                    ...
                                    if(isset($_GET['n_id'])) {
                            $n_id = $_GET['n_id'];
                            $forn_q = "SELECT DISTINCT NoteID,NoteTitle FROM downloads WHERE IsSellerHasAllowedDownload = b'1' AND NoteID = $n_id ";
                        } else {
                            $forn_q = "SELECT DISTINCT NoteID,NoteTitle FROM downloads WHERE IsSellerHasAllowedDownload = b'1' ";
                        }
                        $forn = mysqli_query($connection, $forn_q);
                        if(!$forn) {
                            die("Query Failed".mysqli_error($connection));
                        }
                        while($forn_row = mysqli_fetch_assoc($forn)) {
                            $fors_n = $forn_row['NoteID'];
                            $forn_title = $forn_row['NoteTitle'];
?>
                                    <option value="<?php echo $forn_title; ?>"><?php echo $forn_title; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                        ...
                        if(isset($_GET['n_id'])) {
                            $n_id = $_GET['n_id'];
                            $fors_q = "SELECT DISTINCT Seller FROM downloads WHERE IsSellerHasAllowedDownload = b'1' AND NoteID = $n_id ";
                        } else {
                            $fors_q = "SELECT DISTINCT Seller FROM downloads WHERE IsSellerHasAllowedDownload = b'1' ";
                        }
                        $fors = mysqli_query($connection, $fors_q);
                        if(!$fors) {
                            die("Query Failed".mysqli_error($connection));
                        }
                        while($fors_row = mysqli_fetch_assoc($fors)) {
                            $fors_s = $fors_row['Seller'];
                        ...
                        <div class="form-group col-md">
                            ...
                            <div class="col">
                                ...
                                <select class="form-control select" id="buyer" name="buyer">
                                    ...
                                    if(isset($_GET['n_id'])) {
                            $n_id = $_GET['n_id'];
                            $forb_q = "SELECT DISTINCT Downloader FROM downloads WHERE IsSellerHasAllowedDownload = b'1' AND NoteID = $n_id ";
                        } else { 
                            $forb_q = "SELECT DISTINCT Downloader FROM downloads WHERE IsSellerHasAllowedDownload = b'1' ";
                        }
                        $forb = mysqli_query($connection, $forb_q);
                        if(!$forb) {
                            die("Query Failed".mysqli_error($connection));
                        }
                        while($forb_row = mysqli_fetch_assoc($forb)) {
                            $fors_b = $forb_row['Downloader'];
                            $fors_b_query = mysqli_query($connection,"SELECT * FROM users WHERE ID = '{$fors_b}' ");
                            if(!$fors_b_query) {
                                die("Query Failed".mysqli_error($connection));
                            }
                            $forb_fetch = mysqli_fetch_assoc($fors_b_query);
                            $forb_name = $forb_fetch['FirstName']." ".$forb_fetch['LastName'];
?>
                                    <option value="<?php echo $forb_name; ?>"><?php echo $forb_name; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                ...
                <table class="table table-hover" style="width:100%;">
                    ...
                    <tbody id="mytable">
                        ...
                        if(isset($_POST['search'])) {
                            ...
                            $download_count = "SELECT * FROM downloads WHERE IsSellerHasAllowedDownload = b'1' AND CONCAT(AttachmentDownloadDate,IsPaid,PurchasedPrice,NoteTitle,NoteCategory) LIKE '%$search%' ";
                            $download_query = "SELECT * FROM downloads WHERE IsSellerHasAllowedDownload = b'1' AND CONCAT(AttachmentDownloadDate,IsPaid,PurchasedPrice,NoteTitle,NoteCategory) LIKE '%$search%' ORDER BY AttachmentDownloadDate DESC LIMIT $page1,5 ";
...
?>
                        <tr>
                            ...
                            <td><a href="memberdetails.php?id=<?php echo $buyer; ?>"><img src="images/eye.png"></a></td>
    ...
    <nav id="pagination2" class="publish-p" aria-label="Page navigation example">
        <ul class="pagination">
            <?php
            if($page>=2) {
                   echo '<li id="prev" class="page-item"><a class="page-link" href="download.php?page='.($page - 1).'&source='.$search.'"><img src="images/left-arrow.png" alt="arrow"></a></li>';
            }
            
            for($i = 1; $i <= $page_count; $i++) {
                if($i == $page) {
                    echo '<li class="page-item number active"><a class="page-link" href="download.php?page='.$i.'&source='.$search.'">'.$i.'</a></li>';
                } else {
                    echo '<li class="page-item number"><a class="page-link" href="download.php?page='.$i.'&source='.$search.'">'.$i.'</a></li>';
                }
                
                
            }
            if($page<$page_count)
                   echo '<li id="next" class="page-item"><a class="page-link" href="download.php?page='.($page + 1).'&source='.$search.'"><img src="images/right-arrow.png" alt="arrow"></a></li>';
