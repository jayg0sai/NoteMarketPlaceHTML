...
$for_admin_q = "SELECT * FROM users WHERE ID = '{$_SESSION['userid']}' ";
$for_admin = mysqli_query($connection, $for_admin_q);
if(!$for_admin){
    die("Query Failed".mysqli_error($connection));
}
$admin_row = mysqli_fetch_assoc($for_admin);
$role = $admin_row['RoleID'];
if($role == 1 || $role == 2){
