...
<head>
    ...
    <!-- Custom CSS -->
    <link rel="stylesheet" href="css/mysold.css?v=<?php echo time(); ?>">

    <!-- Responsive CSS -->
    <link rel="stylesheet" href="css/mysold-responsive.css?v=<?php echo time(); ?>">
                ...
                <tbody>
                   <?php
                    ...
                    if(isset($_POST['search'])) {
                        ...
                        $seller_count = "SELECT * FROM downloads WHERE Seller = '{$_SESSION['userid']}' AND IsSellerHasAllowedDownload = b'1' AND (NoteTitle LIKE '%$search%' OR NoteCategory LIKE '%$search%' OR PurchasedPrice LIKE '%$search%') ";
                        $find_count = mysqli_query($connection, $seller_count);
                        if(!$find_count) {
                        ...
                        $download_query = "SELECT * FROM downloads WHERE Seller = '{$_SESSION['userid']}' AND IsSellerHasAllowedDownload = b'1' AND (NoteTitle LIKE '%$search%' OR NoteCategory LIKE '%$search%' OR PurchasedPrice LIKE '%$search%') ORDER BY AttachmentDownloadDate DESC LIMIT $page1,10 ";
...
?>
    ...
    <nav id="pagination" class="download-p" aria-label="Page navigation example">
        <ul class="pagination">
            <?php
            if($page>=2) {
                   echo '<li id="prev" class="page-item"><a class="page-link" href="mysold_search.php?page='.($page - 1).'&source='.$search.'"><img src="images/left-arrow.png" alt="arrow"></a></li>';
            }
            
            for($i = 1; $i <= $page_count; $i++) {
                if($i == $page) {
                    echo '<li class="page-item number active"><a class="page-link" href="mysold_search.php?page='.$i.'&source='.$search.'">'.$i.'</a></li>';
                } else {
                    echo '<li class="page-item number"><a class="page-link" href="mysold_search.php?page='.$i.'">'.$i.'&source='.$search.'</a></li>';
                }
                
                
            }
            if($page<$page_count)
                   echo '<li id="next" class="page-item"><a class="page-link" href="mysold_search.php?page='.($page + 1).'&source='.$search.'"><img src="images/right-arrow.png" alt="arrow"></a></li>';
    ...
    <!-- Custom JS -->
    <script src="js/mysold.js?v=<?php echo time(); ?>"></script>
